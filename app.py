...
summary_stats_db = df_db.describe()

# Display summary statistics
summary_stats_db

df_db.hist(bins=10, figsize=(14, 12), layout=(3, 3), color='#3498db')
plt.tight_layout()
plt.show()

# Set up for correlation matrix
plt.figure(figsize=(12, 8))
corr_matrix_diabetes = df_db.corr()
...
for col in cols_with_zeros:
    df_db[col] = df_db[col].replace(0, df_db[col].median())
...
df_db[features_to_normalize] = scaler.fit_transform(df_db[features_to_normalize])

# Display the first few rows after normalization
df_db.head()

correlation_matrix = df_db.corr().abs()
...
X = df_db.drop('Outcome', axis=1)  # Features
y = df_db['Outcome']               # Target
